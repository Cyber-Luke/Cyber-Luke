...
USER_NAME = os.environ['USER_NAME'] # 'Cyber-Luke'
...
def svg_overwrite(filename, commit_data, star_data, repo_data, follower_data, loc_data):
    """
    Parse SVG file and update elements with commits, stars, repositories, followers, and lines of code
    """
    tree = etree.parse(filename)
    root = tree.getroot()
    
    # Update all data fields in animated.svg
    find_and_replace(root, 'commit_data', f"{'{:,}'.format(commit_data)}")
    find_and_replace(root, 'star_data', f"{'{:,}'.format(star_data)}")
    find_and_replace(root, 'repo_data', str(repo_data))
    find_and_replace(root, 'follower_data', f"{'{:,}'.format(follower_data)}")
...
if __name__ == '__main__':
    """
    Andrew Grant (Cyber-Luke), 2022-2025
    """
    ...
    svg_overwrite('animated.svg', commit_data, star_data, repo_data, follower_data, total_loc[:-1])

    # move cursor to override 'Calculation times:' with 'Total function time:' and the total function time, then move cursor back
    print('\033[F\033[F\033[F\033[F\033[F\033[F\033[F',
        '{:<21}'.format('Total function time:'), '{:>11}'.format('%.4f' % (user_time + loc_time + commit_time + star_time + repo_time + follower_time)),
